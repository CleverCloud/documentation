...
Go instances include additional runtime environments such as [Node.js](/doc/deploy/applications/nodejs) that you can use through scripts launched by [deployment hooks](/doc/develop/common-configuration/build-hooks). Some of these runtimes can be configured through environment variables. For example, to use a specific version of Node.js, set `CC_NODE_VERSION` (it could be `node` (latest), `lts/*`, `24` or `24.13.0`).
