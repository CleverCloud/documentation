...
For each Pulsar add-on we provide, we also provide a hidden [Cellar add-on]({{< ref "doc/deploy/addon/cellar.md" >}}), our object storage add-on. This Cellar add-on is bound to the Pulsar namespace offload policies and will store your offloaded data.
