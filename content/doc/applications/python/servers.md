...
| Name | Description | Default |
|------|-------------|---------|
| `GUNICORN_WORKER_CLASS` | Type of worker to use. [Available workers](https://docs.gunicorn.org/en/stable/settings.html#worker-class) | `sync` |
| `CC_GUNICORN_TIMEOUT` | Gunicorn timeout | `30` |
...
> [!WARNING]
> Setting the `STATIC_URL_PREFIX` to `/` will cause the deployment failure.
