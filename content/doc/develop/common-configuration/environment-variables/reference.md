...
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------|
| [`INSTANCE_NUMBER`](/doc/develop/common-configuration/environment-variables/#what-is-the-instance_number-variable-used-for "What is the INSTANCE_NUMBER variable used for?") | Allows your application to differentiate each running node on the application level.                                                                                                         | 0, 1…                                                               |
...
| [`CC_HEALTH_CHECK_PATH`](/doc/develop/common-configuration/healthcheck) | Custom path to validate your application deployment                                                                                    |               |
...
| [`CC_WORKER_COMMAND`](/doc/develop/common-configuration/workers)        | Command to run in background as a worker process. You can run multiple workers.                                                        |               |

You can set some tools' version in any runtime (except Docker):

| Name              | Description                                                 |
| ----------------- | ----------------------------------------------------------- |
| `CC_HUGO_VERSION` | Set the Hugo version, for example `0.161`                   |
...
| [`CC_DISABLE_BUILD_CACHE_UPLOAD`](/doc/develop/common-configuration/environment-variables/#settings-you-can-define-using-environment-variables "Settings you can define using environment variables") | Disable creation and upload of cache archive. Restarts won't be speeded up.                                                                                                                                                                                                                    | `false`       |
| [`CC_IGNORE_FROM_BUILDCACHE`](/doc/develop/common-configuration/environment-variables/#settings-you-can-define-using-environment-variables "Settings you can define using environment variables")     | Allows to specify paths to ignore when the build cache archive is created.                                                                                                                                                                                                                     |               |
| [`IGNORE_FROM_BUILDCACHE`](/doc/develop/common-configuration/environment-variables/#settings-you-can-define-using-environment-variables "Settings you can define using environment variables")        | (Deprecated) Allows to specify paths to ignore when the build cache archive is created.                                                                                                                                                                                                        |               |
| [`CC_OVERRIDE_BUILDCACHE`](/doc/develop/common-configuration/environment-variables/#settings-you-can-define-using-environment-variables "Settings you can define using environment variables")        | Allows to specify paths that will be in the build cache. Only those files / directories will be cached                                                                                                                                                                                         |               |
...
{{< callout emoji="🐳" >}}
  `CC_RUN_COMMAND` has no effect on Docker. To run Docker, use `CMD` in your [Dockerfile](/doc/deploy/applications/docker/#dockerfile-contents).
{{< /callout >}}

#### Deployment hooks

Use these to define [commands to run](/doc/develop/common-configuration/build-hooks) between various steps of the deployment.

| Name                                                                                                                   | Description                                                                                                               |
|------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| [`CC_PRE_BUILD_HOOK`](/doc/develop/common-configuration/build-hooks/#pre-build "Pre build hook")                       | Ran before the dependencies are fetched. If it fails, the deployment fails.                                               |
| [`CC_POST_BUILD_HOOK`](/doc/develop/common-configuration/build-hooks/#post-build "Post build hook")                    | Ran after the project is built, and before the cache archive is generated. If it fails, the deployment fails.             |
| [`CC_PRE_RUN_HOOK`](/doc/develop/common-configuration/build-hooks/#pre-run "Pre run hook")                             | Ran before the application is started, but after the cache archive has been generated. If it fails, the deployment fails. |
| [`CC_RUN_FAILED_HOOK`](/doc/develop/common-configuration/build-hooks/#run-successfail "Run succeeded/failed hook")     | Ran once the application has failed to start.                                                                             |
| [`CC_RUN_SUCCEEDED_HOOK`](/doc/develop/common-configuration/build-hooks/#run-successfail "Run succeeded/failed hook" ) | Ran once the application has started successfuly.                                                                         |

#### Configure extra software

| Name                                                                                                                                | Description                                                                                                                                                                                                           | Default value              |
|-------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
| [`CC_CLAMAV`](/doc/security/clamav "Clamav")                                                                                        | Start the clamav and clamav-freshclam services (the database is updated every 2 hours). WARNING: Clamscan consumes a lot of resources (~ 1GB of memory), make sure you have a scaler with enough memory to avoid OOM. | false                      |
| [`CC_CLAMAV_MAXQUEUE`](/doc/security/clamav "Clamav")                                                                               | Maximum number of queued items.                                                                                                                                                                                       | 100                        |
| [`CC_CLAMAV_MAXTHREADS`](/doc/security/clamav "Clamav")                                                                             | Maximum number of threads running at the same time.                                                                                                                                                                   | 10                         |
| [`CC_METRICS_PROMETHEUS_PASSWORD`](/doc/develop/observability/metrics/#publish-your-own-metrics "Publish your own metrics")         | Define the password for the basic auth of the Prometheus endpoint                                                                                                                                                     |                            |
| [`CC_METRICS_PROMETHEUS_PATH`](/doc/develop/observability/metrics/#publish-your-own-metrics "Publish your own metrics")             | Define the path on which the Prometheus endpoint is available                                                                                                                                                         | /metrics                   |
| [`CC_METRICS_PROMETHEUS_PORT`](/doc/develop/observability/metrics/#publish-your-own-metrics "Publish your own metrics")             | Define the port on which the Prometheus endpoint is available                                                                                                                                                         | 9100                       |
| [`CC_METRICS_PROMETHEUS_RESPONSE_TIMEOUT`](/doc/develop/observability/metrics/#publish-your-own-metrics "Publish your own metrics") | Define the timeout in seconds to collect the application metrics. This value **must** be below 60 seconds as data are collected every minutes                                                                         | 3                          |
| [`CC_METRICS_PROMETHEUS_USER`](/doc/develop/observability/metrics/#publish-your-own-metrics "Publish your own metrics")             | Define the user for the basic auth of the Prometheus endpoint                                                                                                                                                         |                            |
...
| [`CC_VARNISH_FILE`](/doc/develop/request-flow/varnish "Cache")                                                                      | The path to the Varnish configuration file, relative to your application root                                                                                                                                         | `/clevercloud/varnish.vcl` |
| [`CC_VARNISH_STORAGE_SIZE`](/doc/develop/request-flow/varnish "Cache")                                                              | Configure the size of the Varnish cache.                                                                                                                                                                              | 1G                         |
| [`CC_WORKER_COMMAND`](/doc/develop/common-configuration/workers "Workers")                                                          | Command to run in background as a worker process. You can run multiple workers.                                                                                                                                       |                            |
...
For a given application with `TAILSCALE_AUTH_KEY` configured, each instance will be configured to join a Tailscale network. Instances will be named after your configured name, suffixed with the [INSTANCE_NUMBER](/doc/develop/common-configuration/environment-variables "Environment variables") : `CC-<NAME>-<INSTANCE_NUMBER>`. If you have multiple instances and use one of them for being an admin instance (using [INSTANCE_NUMBER](/doc/develop/common-configuration/environment-variables "Environment variables")), you can match the instance from your deployment to reach it over VPN.
...
[Docker Documentation](/doc/deploy/applications/docker)

| Name                          | Description                                                              | Default value            |
| ----------------------------- | ------------------------------------------------------------------------ | ------------------------ |
| `CC_DOCKERFILE`               | The name of the Dockerfile to build.                                     | Dockerfile               |
| `CC_DOCKER_BUILDX`            | Set to `false` to use Docker legacy build                                | true                     |
...
[.NET Documentation](/doc/deploy/applications/dotnet)
...
| `CC_DOTNET_VERSION` | Choose the .NET Core version between `6.0`,`8.0`, `9.0`.                                                 | 8.0           |
| `CC_RUN_COMMAND`    | Custom command to run your application.                                                                  |               |

## Elixir

[Elixir Documentation](/doc/deploy/applications/elixir)

| Name                          | Description                                                                                                 | Default value |
|-------------------------------|-------------------------------------------------------------------------------------------------------------|---------------|
| `CC_ELIXIR_VERSION`           | Choose the Elixir version among [those supported](/doc/deploy/applications/elixir/#mandatory-configuration) |               |
...
[FrankenPHP Documentation](/doc/deploy/applications/frankenphp)

| Name                      | Description                                                                                 | Default value                            |
| ------------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------- |
| `CC_FRANKENPHP_PORT`      | The port on which FrankenPHP listens for HTTP requests                                      | `8080`                                   |
| `CC_FRANKENPHP_WORKER`    | Path to the worker script, relative to the root of your project (e.g. `/worker/script.php`) |                                          |
...
[Go Documentation](/doc/deploy/applications/golang)
...
[Haskell Documentation](/doc/deploy/applications/haskell)

| Name                                                                                                                       | Description                                                                                     |
|----------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------|
| [`CC_HASKELL_STACK_TARGET`](/doc/deploy/applications/haskell/#specify-stack-package-target "Specify Stack package target") | Specify Stack package target.                                                                   |
...
- [Java Documentation](/doc/deploy/applications/java)
- [Scala Documentation](/doc/deploy/applications/scala)
...
[Linux Documentation](/doc/deploy/applications/linux)
...
[Node.js Documentation](/doc/deploy/applications/nodejs)

| Name                                                                    | Description                                                                                              | Default value      |
|-------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|--------------------|
| [`CC_NODE_VERSION`](/doc/deploy/applications/nodejs#set-nodejs-version) | Set Node.js version, for example `24`, `23.11` or `22.15.1`                                              |                    |
...
[PHP Documentation](/doc/deploy/applications/php)

| Name                                                                                                                                                                  | Description                                                                                                                                                                                                                         | Default value                                                    |
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------|
| `ALWAYS_POPULATE_RAW_POST_DATA`                                                                                                                                       |                                                                                                                                                                                                                                     |                                                                  |
| `CC_COMPOSER_VERSION`                                                                                                                                                 | Choose your composer version between `2` or `lts`                                                                                                                                                                                   | 2                                                                |
| [`CC_CONFIGURATION_PM_MAX_CHILDREN`](/doc/deploy/applications/php/#memory-limit)                                                                                      | Set the number of PHP-FPM workers. On XS and larger flavors, increasing it can lower the automatically calculated memory limit. Nano always uses six workers                                                                        | Depends on the scaler size                                       |
| [`CC_CGI_IMPLEMENTATION`](/doc/deploy/applications/php/apache/#change-the-fastcgi-module)                                                                             | Choose the Apache FastCGI module between `fastcgi` and `proxy_fcgi`                                                                                                                                                                 | proxy_fcgi                                                       |
...
| [`CC_PHP_ASYNC_APP_BUCKET`](/doc/deploy/applications/php/sessions-emails/#speed-up-or-disable-the-session-fs-bucket "Speed up or disable the session on FS Bucket")   | Mount the default app FS bucket asynchronously. If set, should have value `async`                                                                                                                                                   |                                                                  |
| [`CC_PHP_DEV_DEPENDENCIES`](/doc/deploy/applications/php/composer/#development-dependencies "Development dependencies")                                               | Set to `install` to include dev dependencies; unset or set to `skip` to exclude them                                                                                                                                                |                                                                  |
| [`CC_PHP_DISABLE_APP_BUCKET`](/doc/deploy/applications/php/sessions-emails/#speed-up-or-disable-the-session-fs-bucket "Speed up or disable the session on FS Bucket") | Disable entirely the app FS Bucket. Values are either `true`, `yes` or `disable`                                                                                                                                                    |                                                                  |
| `CC_PHP_VERSION`                                                                                                                                                      | Choose your PHP version among [those supported](/doc/deploy/applications/php/#php-version)                                                                                                                                          | 8.3                                                              |
...
| `LDAPTLS_CACERT`                                                                                                                                                      |                                                                                                                                                                                                                                     |                                                                  |
| `MAX_INPUT_VARS`                                                                                                                                                      |                                                                                                                                                                                                                                     |                                                                  |
| [`MEMORY_LIMIT`](/doc/deploy/applications/php/#memory-limit)                                                                                                          | Set the maximum memory each PHP script can allocate, as a positive integer interpreted in MiB without a unit suffix                                                                                                                 | Depends on the scaler size                                       |
| [`SESSION_TYPE`](/doc/deploy/applications/php/sessions-emails/#use-materia-kv-or-redis-to-store-php-sessions "Use Materia KV or Redis to store PHP sessions")         | Choose `redis` to use it as session store                                                                                                                                                                                           |                                                                  |
| `SOCKSIFY_EVERYTHING`                                                                                                                                                 |                                                                                                                                                                                                                                     |                                                                  |
...
[Python Documentation](/doc/deploy/applications/python)
...
| [`CC_PYTHON_MANAGE_TASKS`](/doc/deploy/applications/python/#django-managepy-tasks)                                                | Comma-separated list of Django manage tasks                                                                                                                                                                  |                                                                                                                    |
...
| `CC_PYTHON_VERSION`                                                                                                               | Choose the Python version among [those supported](/doc/deploy/applications/python/#python-version)                                                                                                           | 3                                                                                                                  |
...
| [`STATIC_URL_PREFIX`](/doc/deploy/applications/python/#configure-your-python-application "Configure your Python application")     | The URL path under which you want to serve static file, usually /public                                                                                                                                      |                                                                                                                    |
| `STATIC_WEBROOT`                                                                                                                  |                                                                                                                                                                                                              |                                                                                                                    |
| [`UWSGI_ASYNC`](/doc/deploy/applications/python/#configure-your-python-application "Configure your Python application" )          | Number of cores to use for uWSGI asynchronous/non-blocking modes                                                                                                                                             |                                                                                                                    |
...
- [Ruby Documentation](/doc/deploy/applications/ruby)

| Name                                                                                      | Description                                                                                                                                                                                                                                | Default value                                                                                                      |
|-------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------|
| [`CC_ENABLE_SIDEKIQ`](/doc/deploy/applications/ruby/#configure-sidekiq)                   | Enable Sidekiq background process                                                                                                                                                                                                          | false                                                                                                              |
| `CC_HTTP_BASIC_AUTH`                                                                      | Restrict HTTP access to your application. Example: `login:password`. You can define multiple credentials using additional `CC_HTTP_BASIC_AUTH_n` (where `n` is a number) environment variables.                                            |                                                                                                                    |
...
| [`CC_RAKEGOALS`](/doc/deploy/applications/ruby/#configure-rake-goals)                     | A list of comma-separated rake goals to execute e.g. db:migrate, assets:precompile                                                                                                                                                         |                                                                                                                    |
...
| `RAILS_ENV`                                                                               |                                                                                                                                                                                                                                            |                                                                                                                    |
| `STATIC_FILES_PATH`                                                                       | Relative path to where your static files are stored: path/to/static                                                                                                                                                                        |                                                                                                                    |
| [`STATIC_URL_PREFIX`](/doc/deploy/applications/ruby/#manage-your-static-files-and-assets) | The URL path under which you want to serve static file, usually /public                                                                                                                                                                    |                                                                                                                    |
| `STATIC_WEBROOT`                                                                          |                                                                                                                                                                                                                                            |                                                                                                                    |

## Rust

[Rust Documentation](/doc/deploy/applications/rust)
...
[Static Documentation](/doc/deploy/applications/static)
...
> If no `CC_BUILD_COMMAND` is provided, Clever Cloud tries to detect and configure Static Site Generator (SSG), any provided `CC_WEBROOT` is ignored. See [Static Site Generators (SSG) Auto-build](/doc/deploy/applications/static/#static-site-generators-ssg-auto-build) for more information.

## V (Vlang)

[V (Vlang)](/doc/deploy/applications/v)
...
[FS Bucket Documentation](/doc/deploy/storage/fs-bucket)
...
[MongoDB Documentation](/doc/deploy/databases/mongodb)
...
[MySQL Documentation](/doc/deploy/databases/mysql)
...
[PostgreSQL Documentation](/doc/deploy/databases/postgresql)
...
| `CC_PGPOOL_SOCKET_PATH`                           | Contains the path to the Unix domain socket used to connect to Pgpool-II                                                                                                                          | `/var/run/pgpool`    |
...
| `CC_PGPOOL_HEALTH_CHECK_PERIOD`                   | Health check period                                                                                                                                                                               | 5                    |
| `CC_PGPOOL_HEALTH_CHECK_TIMEOUT`                  | Health check timeout                                                                                                                                                                              | 20                   |
| `CC_PGPOOL_HEALTH_CHECK_MAX_RETRIES`              | Maximum number of times to retry a failed health check before giving up                                                                                                                           | 8                    |
| `CC_PGPOOL_HEALTH_CHECK_RETRY_DELAY`              | Amount of time to wait (in seconds) between retries                                                                                                                                               | 5                    |
...
[Elastic Stack Documentation](/doc/deploy/databases/elastic)
...
[Redis Documentation](/doc/deploy/databases/redis)
...
> [!NOTE]
> New setups use the `CC_VPN_*` names listed below. The previous `VPN_ADDON_*` names (and `VPN_TARGETS`) are deprecated and kept only for backward compatibility with existing setups.
