...
- Replace the line `$settings['hash_salt'] = ''` (`$drupal_hash_salt` for Drupal 7) with `$settings['hash_salt'] = getenv('DRUPAL_SALT')`. You can generate salts with [this link](https://www.passwordtool.hu/). Add a new `DRUPAL_SALT` [environment variable](/doc/deploy/applications/php#configure-your-php-application) to the application with the salt you have generated.
- As mentioned in this [article](/doc/deploy/storage/fs-bucket), with Git deployments, files that are uploaded by users must be
persisted in a File System Bucket. In order to do so, [add a File Bucket](/doc/deploy/storage/fs-bucket) via the console.
